...
- `responses_total` — meaningful approver response rows only
  (`date_status_type != "NOT_CALLED"` after stage_normalize). This is the count
  of actual called-approver responses, not the raw GED unpivot size.
- `responses_cells_total` — raw size of the GED wide→long unpivot
  (`docs_total × approver_column_groups`). Kept for traceability and to detect
  changes in the GED export shape (e.g. if approver columns are added/removed).
|---|---:|
| `docs_total` | 6491 |
