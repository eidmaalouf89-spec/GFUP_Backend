...
## Phase 6A — Counter-Attack Action Kernel (NOT a pipeline stage in 6A)

`output/intermediate/COUNTER_ATTACK_ITEMS.csv` is **not produced by any
pipeline stage** in Phase 6A. It is generated by a standalone script and is
intentionally outside the run lifecycle.

- **Producer (CLI):** `scripts/build_counter_attack.py`
- **Builder:** `src.reporting.counter_attack_builder.build_counter_attack_items(ctx, output_dir)`
- **RunContext:** `reporting.data_loader.load_run_context(BASE_DIR)` (the same loader the UI uses, no run-number argument required).
- **Read-only inputs:**
  - `compute_dcc_tags_bulk(ctx)` from `reporting.document_command_center` (additive bulk wrapper around the existing DCC private helpers — no new tag rules)
  - `output/chain_onion/CHAIN_REGISTER.csv`, `CHAIN_METRICS.csv`, `ONION_SCORES.csv`, `CHAIN_NARRATIVES.csv`
  - `output/intermediate/CHAIN_TIMELINE_ATTRIBUTION.csv`
  - `ctx.responses_df` (for evidence: observations across all indices + consultant reports via `effective_source`)

**What 6A intentionally does NOT do:**
- No `stage_finalize_run` integration. The artifact is not registered in `data/run_memory.db`.
- No new pipeline stage; runner.py and the eleven existing stages are untouched.
- No DB schema change. `run_memory.db` and `report_memory.db` are unchanged.
- No UI/API change. `app.py`, `data_bridge.js`, and `ui/` are untouched.
- No mutation of any existing artifact contract.

**Run after a pipeline run.** Pipeline produces FLAT_GED + chain_onion outputs as usual; once those exist, run `python scripts/build_counter_attack.py` to (re)generate the counter-attack artifact deterministically. Same inputs → byte-identical CSV.

Pipeline-stage integration is deferred to a future phase if/when consumers (Phase 6B read API, Phase 6C UI) need it tied into `stage_finalize_run`.
