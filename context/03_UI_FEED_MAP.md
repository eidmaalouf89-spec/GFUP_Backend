...
| Counter-Attack — bucket queue (Phase 6B) | `get_counter_attack_queue(bucket, limit=500)` | `reporting.counter_attack_query.get_counter_attack_queue` (same artifact; bucket display order post-2026-05-11: FERMER_MAINTENANT, DECISION_MOEX, ENTREPRISE_A_RELANCER, CONSULTANT_A_ATTAQUER. Queue row payloads include `warning_tags` field.) |
