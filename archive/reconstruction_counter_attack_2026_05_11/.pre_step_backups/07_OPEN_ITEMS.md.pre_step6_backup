...
## Open items from 2026-05-10 orchestration session

The orchestration session (Steps 1, 1.5, 2, 3, 4, 5) shipped Steps 1, 1.5,
2, 3, 5; Step 4 was rolled back. Open follow-ups recorded below.

### OS-2026-05-10-01 — Action MOEX bucket-count baseline drift (845 → 1116) — open investigation

When the user reran `python scripts/build_counter_attack.py` after the
Step 4 cascade landed, the artifact went from a Step-0-recorded baseline
of 845 rows to 1116 rows. The cascade was rolled back; the drift
remained. The baseline must be reconciled before any further Action
MOEX bucket-routing work.
...
User confirmed: no upstream rerun (no `python main.py`, no
`python run_chain_onion.py`); chain+onion CSVs were not re-emitted.

**Diagnostic conclusion (read-only investigation by Read/Grep tools).**
The Step 4 cascade alone could NOT have caused these deltas:
...
- Cascade is correctly placed as the LAST conditional in
...
- 0 of the 49 FERMER_MAINTENANT rows came from the cascade. All 49
  routed via Step 5 / 5b.
- 0 of the 212 ENTREPRISE_A_RELANCER rows came from the cascade. All
  212 routed via Step 2 / 3.

**Most plausible explanation (not proven):** the 845-row baseline was
generated by an earlier code revision than what was on disk at Step 4
backup time. Evidence: the repo carries historical reconstruction copies
in `src/reporting/` named
`counter_attack_builder.RECONSTRUCTION_R1_PREWRITE.py`,
`counter_attack_builder.RECONSTRUCTION_R1_VALIDATED.py`, and
`counter_attack_builder.R2C_PREWRITE.py`. The 2026-05-04 entry in
`context/11_TOOLING_HAZARDS.md` change log documents an extensive Phase
6X.F2-bis R1/R2/R3 reconstruction done outside Cowork (by Codex) when
the file was corrupted.

**Suggested investigation starting points:**

1. Compare current `_assign_bucket` against
   `counter_attack_builder.RECONSTRUCTION_R1_VALIDATED.py` and
   `counter_attack_builder.R2C_PREWRITE.py` to identify which historical
   version produced 845 rows.
2. Confirm whether `build_counter_attack.py` output was last regenerated
   days/weeks ago vs today. (mtime on the artifact CSV pre-rebuild
   would have answered this; not preserved.)
3. Determine whether the operational baseline the project depends on is
   845 or 1116 — and which is "correct" given current upstream
   chain+onion data.

Until this is resolved, do NOT reopen the SUJET_REUNION removal cascade
or any other bucket-routing change.

### OS-2026-05-10-02 — Em-dash narrowness pattern (cascade defect, ≤15 rows)

The Step 4 cascade had one bounded defect even before rollback: its
`primary_tag` literal set matched only the U+2014 em-dash variant, while
the rest of `counter_attack_builder.py`'s tag predicates
(`MOEX_FACILE_TAGS`, `MOEX_ARBITRAGE_TAGS`, `_is_contractor_tag`) accept
three byte-variants (hyphen, em-dash, mojibake). This affected ≤15
high-urgency MOEX_SHAME rows max — NOT the observed 491 delta in
OS-2026-05-10-01.

**Forward rule:** if a future Action MOEX cascade or any other
`primary_tag` matching is needed, MUST reuse the existing helpers
`_is_moex_facile()`, `_is_moex_arbitrage()`, `_is_contractor_tag()`,
`_is_primary_tag()`, `_is_secondary_tag()` (lines 285-302 of
`counter_attack_builder.py`) rather than inline literal sets — they
already handle three byte-variants per tag.

### OS-2026-05-10-03 — `_get_latest_responses_for_doc` private-symbol coupling

`src/reporting/counter_attack_export.py` (Step 5) imports
`_get_latest_responses_for_doc` from
`src/reporting/document_command_center.py` directly. The DCC helper is
private (leading underscore). Imported because protected-zone rules
(`obsidian_repo_mind/12_DONT_TOUCH_WITHOUT_EXPLICIT_SCOPE.md`) forbid
modifying DCC. If DCC ever renames or repackages this helper, the
Action MOEX bucket export breaks silently.

**Suggested resolution (one of):**

- Formalize a public re-export from DCC, e.g. add
  `get_latest_responses_for_doc = _get_latest_responses_for_doc` as a
  named public alias in `document_command_center.py`, and have
  `counter_attack_export.py` import the public name. (Lowest risk.)
- Add a regression test that imports the symbol and asserts its
  signature — fails loudly the moment DCC drifts.

### OS-2026-05-10-04 — Action MOEX dashboard sub-row clickability

Step 2 wired clicks on 6 operational dashboard tiles (T1–T6) and 4
priority cells (P1–P4). T4 (MOEX Frais/Stale) and T5 (PRIMAIRE/SECONDARY)
sub-rows are NOT yet clickable. The backend already supports `scope`
(values `fresh`/`stale`) and `tier` (values `PRIMARY`/`SECONDARY`) params
on the new `operational_moex` and `operational_consultants` drilldown
kinds. Optional follow-up — surfaces are inert text inside the
now-clickable parent. Best Consultant + Best Entreprise are deliberately
NOT changed (kept as fiche navigation per user request).

---

## Phase 9 — `dernier_df` retirement / `latest_chain_df` adoption — CLOSED 2026-05-11

**Scope.** Steps 2-9 of the Phase 9 plan migrated all HIGH and
MEDIUM-risk operational consumers of `ctx.dernier_df` to the new
canonical `latest_enriched_view(ctx)` (sourced from
`ctx.latest_chain_df` × `ctx.dernier_df` intersection). See `README.md`
§Phase 9 for the full migration summary and stable invariants.

**Key artifacts produced.**
- `src/reporting/latest_chain_view.py` — two public functions:
  `build_latest_chain_view(base_dir, docs_df=None)` and
  `latest_enriched_view(ctx)`.
- `RunContext.latest_chain_df` field (in `data_loader.py`).
- `reports/STEP1_DERNIER_DF_INVENTORY.md` — per-call-site classification.
- 5 new diagnostic gate suites in `check_latest_chain_view.py`
  (G-LCV-, G-DORM-, G-DCC-, G-AGG-, G-CONS-/G-DRL-/G-FOC-).
- Hazard documentation in `context/11_TOOLING_HAZARDS.md` §H-9.

**Stable invariants (current data, 2026-05-11):**
...
**Watchpoints retained from Step 9.**
1. `_resolve_doc_rows` warning text reads "Suggests flat-GED export
   lag." For numero 253100 the actual cause is the SAS filter
   Decision-3, not lag. The wording is conservative (lag remains a
   possible cause for other numeros that hit the same fallback); see
   `context/06_EXCEPTIONS_AND_MAPPINGS.md` §F-3 Decision-3 SAS filter.
2. The pollution gap reported during loader smoke (Step 3 wording)
   was "1,806" (= dernier_df − latest_chain_df). The
   operationally-meaningful gap is "1,807" (= dernier_df −
   latest_enriched_view) since the latter is what consumers actually
   read. Both are tracked in `check_latest_chain_view.py`.

## Outstanding test baselines (data-version drift, not migration regressions)

21 pre-existing pytest failures identified during the Phase 9
regression pass (Step 9). All are data-version baseline drift —
hardcoded counts from older GED exports, missing fixture CSVs, or
columns added/renamed in newer pipeline versions. **None caused by
the dernier_df → latest_chain_df migration.** Update or skip
deliberately in a future cleanup pass.

| Test file | Symptom | Likely cause |
|---|---|---|
| `tests/test_build_flat_ged_batch_smoke.py` | Missing bentin source exclusions CSV | Test fixture not committed; restore `context/source_exclusions/remaining bentin.csv` for the test environment or skip. |
| `tests/test_audit_counts_lineage.py` (10 tests) | Hardcoded baseline counts (27,261 vs 24,812; 2,819 vs 2,554) | GED export refreshed since baselines were authored; reset baselines or pin a fixture export. |
| `tests/test_cache_meta_v2.py` | Expects schema v2, current is v3 | `CACHE_SCHEMA_VERSION` bumped to v3 in a post-baseline change; update test or accept schema migration. |
| `tests/test_extract_flat_ged_trace.py` | unique_numero baseline mismatch | Same data drift as above. |
| `tests/test_query_hooks.py` | Empty DataFrame missing `family_key` column | Edge case where the fixture builds an empty register; add column even when empty. |
| `tests/test_validation_harness.py` (4 tests, D19) | `severity_raw` column missing | Schema field renamed or removed; sync test expectation. |

Recommended action: triage in one batch as "Phase 9.x — test baseline
refresh"; non-urgent.
