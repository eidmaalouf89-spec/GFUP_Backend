# PATCH 4 — Focus Ownership Resolver (Rules 1-6)

## OBJECTIVE

Create `src/reporting/focus_ownership.py` — a new module that resolves per-document ownership for Focus Mode. Every open document gets exactly one owner (or a small set of same-tier owners). This is called from `data_loader.py` after Patch 3's pre-computed columns.

Then wire it into `data_loader.py` so ownership columns are pre-computed at load time alongside the other focus columns.

## RULES

- CREATE one new file: `src/reporting/focus_ownership.py`
- PATCH one existing file: `src/reporting/data_loader.py` (add ownership computation call)
- Do NOT modify `workflow_engine.py`, `focus_filter.py`, `aggregator.py`, `consultant_fiche.py`, `contractor_fiche.py`, or any UI files
- Do NOT create test files
- Do NOT run `npm run build`

---

## STEP 1 — Create `src/reporting/focus_ownership.py`
...
## STEP 2 — Wire ownership into data_loader.py

In `src/reporting/data_loader.py`, add the import at the top of the file, near the other reporting imports (around line 24):
...
Then, inside `load_run_context`, find the block you added in Patch 3 (the `_precompute_focus_columns` call). It should look like:
...
After applying both steps, run these checks:

### Check 1 — Syntax
```bash
python -c "import ast; ast.parse(open('src/reporting/data_loader.py').read()); print('data_loader.py syntax OK')"
python -c "import ast; ast.parse(open('src/reporting/focus_ownership.py').read()); print('focus_ownership.py syntax OK')"
```
...
from reporting.focus_ownership import compute_focus_ownership, classify_consultant
from reporting.focus_ownership import PRIMARY_CANONICAL, SECONDARY_CANONICAL, MOEX_CANONICAL

# Verify classification
assert classify_consultant('ARCHITECTE') == 'PRIMARY'
assert classify_consultant('BET CVC') == 'PRIMARY'
assert classify_consultant('Bureau de Contrôle') == 'SECONDARY'
assert classify_consultant('BET Acoustique') == 'SECONDARY'
assert classify_consultant('AMO HQE') == 'SECONDARY'
assert classify_consultant(\"Maître d'Oeuvre EXE\") == 'MOEX'
assert classify_consultant('BET Structure') == 'PRIMARY'
assert classify_consultant('BET SPK') == 'PRIMARY'
assert classify_consultant('BET VRD') == 'PRIMARY'

print('All classification assertions passed')
print(f'PRIMARY ({len(PRIMARY_CANONICAL)}): {sorted(PRIMARY_CANONICAL)}')
print(f'SECONDARY ({len(SECONDARY_CANONICAL)}): {sorted(SECONDARY_CANONICAL)}')
print(f'MOEX ({len(MOEX_CANONICAL)}): {sorted(MOEX_CANONICAL)}')
"
```
...
print('load_run_context importable with focus_ownership: OK')
"
```

All 3 must pass. If any fails, fix before reporting. Do NOT create test files.
