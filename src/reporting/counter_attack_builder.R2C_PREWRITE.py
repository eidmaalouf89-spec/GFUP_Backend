...
def _secondary_backlog_age(row: pd.Series) -> int | None:
    if "secondary_consultant_days_remaining" in row.index:
        value = row.get("secondary_consultant_days_remaining")
        if _safe_str(value) != "":
...
def _assign_bucket(row: pd.Series) -> str:
    ...
    if current_state == "WAITING_CORRECTED_INDICE" and _is_contractor_tag(primary_tag):
        return "ENTREPRISE_A_RELANCER"

    if _is_primary_tag(primary_tag):
        remaining, _ = _consultant_days_remaining(row, primary_tag)
        if remaining is not None and remaining < 0:
            return "CONSULTANT_A_ATTAQUER"
        return ""

    if _is_secondary_tag(primary_tag) or _safe_bool(row.get("waiting_secondary_flag")):
        age = _secondary_backlog_age(row)
    ...
    if (
        ...
        and current_state != "CHRONIC_REF_CHAIN"
    ):
...
def _plain_reason(bucket: str, row: pd.Series, primary_actor: str, emetteur_name: str) -> str:
    ...
    if bucket == "ENTREPRISE_A_RELANCER":
        return f"L'entreprise doit resoumettre après refus MOEX. Bloqué depuis {stale} jours."
    if bucket == "CONSULTANT_A_ATTAQUER":
        return f"En attente de {primary_actor} depuis {_days_late(bucket, row)} jours."
    if bucket == "FERMER_MAINTENANT":
        return "Tous les avis sont disponibles. MOEX doit émettre le visa."
    if bucket == "DECISION_MOEX":
        return f"MOEX doit arbitrer les avis bloquants. {driver}".strip()
    if bucket == "SECONDAIRE_EXPIRE":
        return f"Le BET secondaire n'a pas répondu dans la fenêtre. {driver}".strip()
    if bucket == "MOEX_SHAME_INTERNAL":
        return f"Cette chaîne expose MOEX en interne depuis {_safe_int(row.get('moex_wait_days'))} jours. {driver}".strip()
    if bucket == "SUJET_REUNION":
        return f"Sujet à escalader en réunion. {_safe_str(row.get('executive_summary'))}".strip()
    return ""
...
def _days_late(bucket: str, row: pd.Series) -> int:
    ...
    if bucket in {"FERMER_MAINTENANT", "DECISION_MOEX", "MOEX_SHAME_INTERNAL", "SECONDAIRE_EXPIRE"}:
        if bucket == "SECONDAIRE_EXPIRE":
            age = _secondary_backlog_age(row)
            if age is not None:
                return age
        return _safe_int(row.get("moex_wait_days"))
    return _safe_int(row.get("stale_days"))
