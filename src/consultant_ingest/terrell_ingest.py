...
# ---------------------------------------------------------------------------
# File-level date extraction (back-fill for rows with blank DATE_RECEPT)
# ---------------------------------------------------------------------------

_TER_DATE_RE = re.compile(r'\b(\d{2}/\d{2}/\d{4})\b')


def extract_fiche_date(pdf) -> str:
    """
    Extract a representative date from the first page of a Terrell fiche.

    Strategy: find the most frequently occurring dd/mm/yyyy date on page 1.
    For Terrell fiches the fiche emission date appears repeatedly in the
    header/footer and is reliably the dominant date.

    Returns date string (dd/mm/yyyy) or '' if nothing found.
    """
    try:
        text = pdf.pages[0].extract_text() or '' if pdf.pages else ''
        dates = _TER_DATE_RE.findall(text)
        if not dates:
            return ''
    ...
    for pdf_path in pdf_files:
        ...
        try:
            with pdfplumber.open(str(pdf_path)) as pdf:
                # Extract a file-level date from page 1 to use as fallback
                # for rows where column 13 (DATE_RECEPT) is blank
                file_date = extract_fiche_date(pdf)
                file_start_idx = len(all_records)
                ...
                # Back-fill DATE_RECEPT from file-level date where blank
                if file_date:
                    filled = 0
                    for i in range(file_start_idx, len(all_records)):
                        if not all_records[i].get('DATE_RECEPT'):
                            all_records[i]['DATE_RECEPT'] = file_date
                            filled += 1
                    if filled:
                        logger.debug(
                            "%s: back-filled DATE_RECEPT on %d rows from fiche date %s",
                            filename, filled, file_date,
                        )
