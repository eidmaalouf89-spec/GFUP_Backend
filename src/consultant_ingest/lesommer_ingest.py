...
# ---------------------------------------------------------------------------
# Report-level date extraction (for rows where table has no per-row date)
# ---------------------------------------------------------------------------

# The Le Sommer header line reads:
# "17&Co SUIVI DES VISAS ET EXIGENCES ENVIRONNEMENTALES [LOT] - [SECTION] dd/mm/yyyy"
# The date at the end of that line is the report emission date.
_LS_REPORT_DATE_RE = re.compile(
    r'SUIVI DES VISAS[^\n]*?(\d{2}/\d{2}/\d{4})',
    re.IGNORECASE,
)


def extract_report_date(page1_text: str) -> str:
    """
    Extract the Le Sommer report-level emission date from page 1 header text.
    Returns dd/mm/yyyy string or '' if not found.
    """
    m = _LS_REPORT_DATE_RE.search(page1_text)
    return m.group(1) if m else ''
...
def extract_spatial_luminaires(
    ...
    for b, (refs, statut, same_band_cmt) in sorted(ref_info.items()):
        ...
        for ref in refs:
            ...
            commentaire = ' '.join(cmt_parts).strip()[:500]
            ...
            records.append({
                ...
                'INDICE':      ind,
                'STATUT_NORM': statut,
                'DATE_VISA':   '',   # filled from file_date at call site
    ...
    for pdf_path in pdf_files:
        ...
        try:
            with pdfplumber.open(str(pdf_path)) as pdf:
                # Extract report-level date once from page 1 header
                try:
                    pg1_text = pdf.pages[0].extract_text() or '' if pdf.pages else ''
                    file_date = extract_report_date(pg1_text)
                except Exception:
                    file_date = ''
                ...
                # Back-fill file_date into rows that have blank DATE_VISA
                # Only use file_date when row has no date from table extraction
                if file_date:
                    for i in range(file_start_idx, len(all_records)):
                        if not all_records[i].get('DATE_VISA'):
                            all_records[i]['DATE_VISA'] = file_date
