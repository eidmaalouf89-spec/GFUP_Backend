...
def stage_write_gf(ctx, log):
    ...
    # Flat mode: attach pre-computed visa_global override to wf_engine so that
    # any downstream call to engine.compute_visa_global_with_date() returns the
    # authoritative value from flat_ged_doc_meta (fixes SAS REF gap, VP-1).
    # Raw mode: no override attached — engine runs normally.
    if getattr(ctx, "flat_ged_mode", "raw") == "flat":
        try:
            from pipeline.stages.stage_read_flat import get_visa_global
            visa_overrides = {}
            for doc_id in effective_responses_df["doc_id"].dropna().unique():
                v, _ = get_visa_global(ctx, doc_id, wf_engine)
                if v is not None:
                    visa_overrides[doc_id] = v
            wf_engine._flat_visa_override = visa_overrides
            log(f"[flat] visa_global override attached: {len(visa_overrides)} docs with pre-computed visa")
        except Exception as _visa_err:
            log(f"[WARN] flat visa_global override setup failed (non-fatal): {_visa_err}")
