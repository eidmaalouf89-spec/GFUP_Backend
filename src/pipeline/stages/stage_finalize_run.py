...
def stage_finalize_run(ctx, log):
    ...
    if _run_number is not None:
        try:
            # responses_total = meaningful approver response rows only.
            # Excludes NOT_CALLED cells emitted by the GED wide→long unpivot
            # (one cell per doc × approver-column group; most are empty).
            _responses_meaningful = int(
                (responses_df["date_status_type"] != "NOT_CALLED").sum()
            ) if "date_status_type" in responses_df.columns else int(len(responses_df))
            _run_summary = {
                "docs_total": int(len(docs_df)),
                "responses_total": _responses_meaningful,
                "responses_cells_total": int(len(responses_df)),
            ...
            _safe_console_print(f"  [ERROR] Run finalize failed: {_run_finalize_err}")
            try:
                finalize_run_failure(RUN_MEMORY_DB, _run_number,
                                    f"Finalize error: {_run_finalize_err}")
                ctx._ACTIVE_RUN_FINALIZED = True
            except Exception:
                _safe_console_print("  [ERROR] Could not mark run as FAILED in DB")
