...
_SQLITE_WRITE_RETRY_DELAYS = (0.0, 0.2, 0.5, 1.0)
...
def _conn(db_path: str) -> sqlite3.Connection:
    ...
    for attempt, delay in enumerate(_SQLITE_WRITE_RETRY_DELAYS):
        try:
            ...
            last_exc = exc
            msg = str(exc).lower()
            if "locked" in msg and attempt < len(_SQLITE_WRITE_RETRY_DELAYS) - 1:
                time.sleep(delay)
                continue
            raise sqlite3.OperationalError(
                f"Could not open SQLite database {db_path}: {exc}"
            ) from exc
    raise sqlite3.OperationalError(
        f"Could not open SQLite database {db_path}: {last_exc}"
    )
