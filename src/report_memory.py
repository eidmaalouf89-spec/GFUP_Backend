...
CREATE TABLE IF NOT EXISTS persisted_report_responses (
    ...
    deactivated_reason   TEXT    NULL DEFAULT NULL,
...
def _migrate_if_needed(conn: sqlite3.Connection) -> None:
    """
    Apply schema migrations when old schema versions are detected.
    Currently handles:
      - project_id schema (v1): drop and recreate
      - missing deactivated_reason column (v2): ALTER TABLE ADD COLUMN
    """
    ...
    if "project_id" in cols:
        ...
        """)
        return  # Tables will be recreated by DDL below

    # v2 migration: add deactivated_reason column if missing
    cur2 = conn.execute("PRAGMA table_info(persisted_report_responses)")
    prr_cols = [row[1] for row in cur2.fetchall()]
    if prr_cols and "deactivated_reason" not in prr_cols:
        conn.execute(
            "ALTER TABLE persisted_report_responses "
            "ADD COLUMN deactivated_reason TEXT NULL DEFAULT NULL"
        )
        conn.commit()
        logger.info("report_memory: migrated persisted_report_responses — added deactivated_reason column")
...
def deactivate_answered_report_rows(
    db_path: str,
    answered_pairs: list,
) -> int:
    """
    Deactivate report rows where Flat GED has already recorded an ANSWERED state.

    For each (doc_id, approver_canonical) pair in `answered_pairs`, set
    is_active=0 and deactivated_reason='GED_ANSWERED_SUPERSEDES' on all
    matching active rows in persisted_report_responses.

    Rows are never deleted -- deactivation is the only removal mechanism.

    Parameters
    ----------
    db_path        : path to report_memory.db
    answered_pairs : list of (doc_id, approver_canonical) tuples where GED is ANSWERED

    Returns number of rows deactivated.
    """
    if not answered_pairs or not Path(db_path).exists():
        return 0

    conn = sqlite3.connect(db_path)
    deactivated = 0
    try:
        for doc_id, approver in answered_pairs:
            cur = conn.execute(
                """
                UPDATE persisted_report_responses
                SET is_active = 0, deactivated_reason = 'GED_ANSWERED_SUPERSEDES'
                WHERE doc_id = ? AND consultant = ? AND is_active = 1
                """,
                (doc_id, approver),
            )
        ...
        if deactivated:
            logger.debug(
                "deactivate_answered_report_rows: %d rows deactivated (GED_ANSWERED_SUPERSEDES)",
                deactivated,
            )
    finally:
        conn.close()
